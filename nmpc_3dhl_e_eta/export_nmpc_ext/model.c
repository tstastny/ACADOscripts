...
const double d_dot = in[38]-Vsafe*t23;
...
const double t24 = in[11]*in[11];
const double t25 = 1.0/Vsafe;
const double t26 = Vsafe*Vsafe;
const double t27 = alpha*alpha;
const double t28 = in[11]*8.61861E1;
const double t29 = in[11]*t24*2.501023E2;
const double t37 = t24*3.05322E1;
const double t30 = t28+t29-t37;
const double t31 = alpha*5.0996;
const double t32 = t27*(-5.343)+t31+1.7E1/3.2E1;
const double t33 = t26*t32*2.38875E-1;
const double t34 = cos(alpha);
const double t35 = 1.0/t34;
const double t36 = sin(alpha);
const double t38 = t25*t30*t35*t36;
const double t39 = t33+t38;
const double t40 = cos(in[6]);
const double t41 = sin(in[6]);
...
out[1] = e_dot;
out[2] = d_dot;
out[3] = t23*(-9.81E2/1.0E2)+t25*t30*(2.0E1/5.3E1)-t26*(alpha*2.5491E-1+t27*2.7337+6.4105E-2)*9.014150943396226E-2;
out[4] = -t25*(t2*(9.81E2/1.0E2)-t39*t40*(2.0E1/5.3E1));
out[5] = (t25*t39*t41*(2.0E1/5.3E1))/t2;
out[6] = in[8];
out[7] = in[9]*t40-in[10]*t41;
out[8] = in[6]*(-1.24716E1)-in[8]*7.4252+in[10]*1.0069+in[14]*1.24716E1;
out[9] = -t26*(alpha*1.9303E-1+in[7]*1.8359E-1+in[9]*4.6239E-2-in[15]*1.8359E-1-9.4955E-4);
...
const double t5 = 1.0/in[44];
const double t6 = 1.0/norm_rp_ne;
const double t7 = -in[2]+p_d;
...
const double e_ne = t4*tP_e-t3*tP_n;

const double t17 = e_dot*e_dot;
const double t18 = n_dot*n_dot;
const double t19 = t17+t18;

double e_b_ne;
if (t19>in[44]) {
    e_b_ne = sqrt(t19)*in[42];                               
} else {
    e_b_ne = in[42]*in[44]*(1.0/2.0)+in[42]*t5*t19*(1.0/2.0);
}
double sat_e_ne = fabs(e_ne)/e_b_ne;
if (sat_e_ne>1.0) sat_e_ne = 1.0;

const double t8 = sat_e_ne-1.0;
const double t9 = t8*t8;
const double t10 = 3.141592653589793*t9*(1.0/2.0);
const double t11 = cos(t10);
const double t12 = sin(t10);

const double e_d = t7;

double e_b_d;
if (fabs(d_dot)>in[44]) {
    e_b_d = fabs(d_dot)*in[43];                               
} else {
    e_b_d = in[43]*in[44]*(1.0/2.0)+in[43]*t5*fabs(d_dot)*(1.0/2.0);
}
double sat_e_d = fabs(e_d)/e_b_d;
if (sat_e_d>1.0) sat_e_d = 1.0;

const double t13 = sat_e_d-1.0;
const double t14 = t13*t13;
const double t15 = 3.141592653589793*t14*(1.0/2.0);
const double t16 = sin(t15);

const double t20 = alpha-in[39]+in[41];
const double t21 = 1.0/(in[41]*in[41]);
const double t22 = -alpha+in[40]+in[41];

const double t23 = 1.0/e_b_ne;
const double t24 = 1.0/e_b_d;

const double rp_n_unit = -t4*t6;
const double rp_e_unit = -t3*t6;

const double atan2_01 = atan2(rp_e_unit*t11+t12*tP_e, rp_n_unit*t11+t12*tP_n);
...
const double atan2_03 = atan2(-t16*tP_d-sgn_rp*cos(t15), t16*sqrt(tP_e*tP_e+tP_n*tP_n));
const double atan2_04 = atan2(-d_dot, sqrt(t19));
...
const double t5 = 1.0/in[41];
const double t6 = (norm_rp_ne>0.0001) ? 1.0/norm_rp_ne : 0.0;
const double t7 = -in[2]+p_d;
...
double e_b_ne;
if (t19>in[41]) {
    e_b_ne = sqrt(t19)*in[39];                               
} else {
    e_b_ne = in[39]*in[41]*(1.0/2.0)+in[39]*t5*t19*(1.0/2.0);
}
double sat_e_ne = fabs(e_ne)/e_b_ne;
if (sat_e_ne>1.0) sat_e_ne = 1.0;
...
double e_b_d;
if (fabs(d_dot)>in[41]) {
    e_b_d = fabs(d_dot)*in[40];                               
} else {
    e_b_d = in[40]*in[41]*(1.0/2.0)+in[40]*t5*fabs(d_dot)*(1.0/2.0);
}
double sat_e_d = fabs(e_d)/e_b_d;
if (sat_e_d>1.0) sat_e_d = 1.0;
...
const double t20 = alpha-in[36]+in[38];
const double t21 = 1.0/(in[38]*in[38]);
const double t22 = -alpha+in[37]+in[38];
