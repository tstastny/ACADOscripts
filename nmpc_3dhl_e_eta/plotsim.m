...
plot(time,J_rec(:,1));
plot(time,J_rec(:,2));
plot(time,J_rec(:,end-1)); % i_eta_lat
plot(time,J_rec(:,end)); % i_eta_lon
legend('e_{ne}','e_d','i_{\eta_{lat}}','i_{\eta_{lon}}')
ylabel('e [m]')
...
plot(time,(yref(1)*ones(length(time),1)-J_rec(:,11)).^2*Q_output(1)); % e_lat
plot(time,(yref(2)*ones(length(time),1)-J_rec(:,12)).^2*Q_output(2)); % e_lat
plot(time,(yref(3)*ones(length(time),1)-J_rec(:,9)).^2*Q_output(3)); % e_eta_lat
plot(time,(yref(4)*ones(length(time),1)-J_rec(:,10)).^2*Q_output(4)); % e_eta_lon
plot(time,(yref(5)*ones(length(time),1)-X_rec(:,4)).^2*Q_output(5)); % e_V
plot(time,(yref(6)*ones(length(time),1)-X_rec(:,9)).^2*Q_output(6)); % e_p
plot(time,(yref(7)*ones(length(time),1)-X_rec(:,10)).^2*Q_output(7)); % e_q
plot(time,(yref(8)*ones(length(time),1)-X_rec(:,11)).^2*Q_output(8)); % e_r
plot(time,(yref(9)*ones(length(time),1)-J_rec(:,13)).^2*Q_output(9)); % alpha_soft

legend('e_{e_{lat}}','e_{e_{lon}}','e_{\eta_{lat}}','e_{\eta_{lon}}','e_V','e_p','e_q','e_r','\alpha_{soft}')
ylabel('J')
