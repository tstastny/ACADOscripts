...
const double n_dot = in[25]+in[6]*cos(in[2]);
const double e_dot = in[26]+in[6]*sin(in[2]);
...
out[0] = n_dot;
out[1] = e_dot;
out[2] = (tan(in[5])*(9.81E2/1.0E2))/in[6];
out[3] = Td_n*(d_e-in[1])-Td_e*(d_n-in[0]);
out[4] = e_chi;
...
out[0] = Td_n*(d_e-in[1])-Td_e*(d_n-in[0]);
out[1] = e_chi;
out[2] = in[3];
out[3] = in[4];
out[4] = in[5];
...
bool check_curve_seg( const double *pos, const double *pparams, const double n_dot, const double e_dot ) {
    ...
    return ( pb_t < 0.0 && dot_T_V > 0.8 && sqrt(bp_n*bp_n+bp_e*bp_e) < 25.0 );
}
