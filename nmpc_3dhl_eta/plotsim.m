...
plot(time,(yref(1)*ones(length(time),1)-J_rec(:,9)).^2*Q_output(1)); % eta_lat
plot(time,(yref(2)*ones(length(time),1)-J_rec(:,10)).^2*Q_output(2)); % eta_lon
plot(time,(yref(3)*ones(length(time),1)-X_rec(:,4)).^2*Q_output(3)); % V
plot(time,(yref(4)*ones(length(time),1)-X_rec(:,9)).^2*Q_output(4)); % p
plot(time,(yref(5)*ones(length(time),1)-X_rec(:,10)).^2*Q_output(5)); % q
plot(time,(yref(6)*ones(length(time),1)-X_rec(:,11)).^2*Q_output(6)); % r
plot(time,(yref(7)*ones(length(time),1)-J_rec(:,13)).^2*Q_output(7)); % alpha_soft

legend('\eta_{lat}','\eta_{lon}','V','p','q','r','\alpha_{soft}')
ylabel('J(x,u)')
