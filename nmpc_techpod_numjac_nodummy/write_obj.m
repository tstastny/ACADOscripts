...
f_lines{k} = 'void lsq_obj_eval( real_t *in, real_t *out ){';
k = k + 1;
f_lines{k} = '';
k = k + 1;

% open obj c file
fid = fopen('ext_lsq_obj_.c');
tline = fgets(fid);
...
kk = 1;
for i = idx_tracked_expr:idx_tracked_expr+n_Y+n_Z-1
...
f_lines{k} = 'void evaluateLSQ( const real_t *in, real_t *out ){';
k = k + 1;
f_lines{k} = '';
k = k + 1;
