% / / / / / / / / / / / / / / / / / / / / / / / / / / / / / / / / / / / / /
% symbolic jacobian for height cost (cubic formulation)
% / / / / / / / / / / / / / / / / / / / / / / / / / / / / / / / / / / / / /
...
% cost
sig_h_expr = ((delta_h - (-r_d - h_terr - h_offset)) / delta_h)^3;

% jacobian
jac_sig_h_cubic = jacobian(sig_h_expr, [r_n; r_e; r_d; xi]);

%% substitute calculations we'll have already done
sig_h = sym('sig_h','real');
jac_sig_h_cubic = subs(jac_sig_h_cubic , sig_h_expr, sig_h);
h12 = sym('h12','real');
jac_sig_h_cubic = subs(jac_sig_h_cubic , h12_expr, h12);
h34 = sym('h34','real');
jac_sig_h_cubic = subs(jac_sig_h_cubic , h34_expr, h34);
dn = sym('dn','real');
jac_sig_h_cubic = subs(jac_sig_h_cubic , dn_expr, dn);
de = sym('de','real');
jac_sig_h_cubic = subs(jac_sig_h_cubic , de_expr, de);

%% get input arguments (used variables)
input_arg = sym2cell(symvar(jac_sig_h_cubic));

%% export to m code
if 1
matlabFunction(jac_sig_h_cubic,'File','jac_sig_h_cubic.m','Vars',input_arg,'Outputs',{'out'});
end

%% export to c code
if 1
ccode(jac_sig_h_cubic,'file','jac_sig_h_cubic_ccode.c');
end

%% prep c code for mpc model functions
if 1
fid = fopen('jac_sig_h_cubic_ccode.c');
...
fid = fopen('jac_sig_h_cubic_ccode.c','w');
for k = 1:length(txt)
