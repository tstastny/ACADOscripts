...
sig_r_tl = ((delta_r_expr + r_offset - r_tl_expr)/delta_r_expr)^3;
sig_r_br = ((delta_r_expr + r_offset - r_br_expr)/delta_r_expr)^3;

% jacobian of radial cost
jac_sig_r_tl_cubic = jacobian(sig_r_tl, [r_n; r_e; r_d; v; gamma; xi]);
jac_sig_r_br_cubic = jacobian(sig_r_br, [r_n; r_e; r_d; v; gamma; xi]);

%% substitute calculations we'll have already done
sig_r = sym('sig_r','real');
jac_sig_r_tl_cubic = subs(jac_sig_r_tl_cubic , sig_r_tl, sig_r);
jac_sig_r_br_cubic = subs(jac_sig_r_br_cubic , sig_r_br, sig_r);
d_occ = sym('d_occ','real');
jac_sig_r_tl_cubic = subs(jac_sig_r_tl_cubic , r_tl_expr, d_occ);
jac_sig_r_br_cubic = subs(jac_sig_r_br_cubic , r_br_expr, d_occ);
delta_r = sym('delta_r','real');
jac_sig_r_tl_cubic = subs(jac_sig_r_tl_cubic , delta_r_expr, delta_r);
jac_sig_r_br_cubic = subs(jac_sig_r_br_cubic , delta_r_expr, delta_r);
vG = sym('vG','real');
jac_sig_r_tl_cubic = subs(jac_sig_r_tl_cubic , vG_expr, vG);
jac_sig_r_br_cubic = subs(jac_sig_r_br_cubic , vG_expr, vG);
...
jac_sig_r_tl_cubic = subs(jac_sig_r_tl_cubic , vG^2, vG_sq);
jac_sig_r_br_cubic = subs(jac_sig_r_br_cubic , vG^2, vG_sq);
% vG = sym('vG','real');
jac_sig_r_tl_cubic = subs(jac_sig_r_tl_cubic , vG_sq^(1/2), vG);
jac_sig_r_br_cubic = subs(jac_sig_r_br_cubic , vG_sq^(1/2), vG);
vG_n = sym('vG_n','real');
jac_sig_r_tl_cubic = subs(jac_sig_r_tl_cubic , vG_n_expr, vG_n);
jac_sig_r_br_cubic = subs(jac_sig_r_br_cubic , vG_n_expr, vG_n);
vG_e = sym('vG_e','real');
jac_sig_r_tl_cubic = subs(jac_sig_r_tl_cubic , vG_e_expr, vG_e);
jac_sig_r_br_cubic = subs(jac_sig_r_br_cubic , vG_e_expr, vG_e);
vG_d = sym('vG_d','real');
jac_sig_r_tl_cubic = subs(jac_sig_r_tl_cubic , vG_d_expr, vG_d);
jac_sig_r_br_cubic = subs(jac_sig_r_br_cubic , vG_d_expr, vG_d);

%% export to m code
if 1
matlabFunction(jac_sig_r_tl_cubic,'File','jac_sig_r_tl_cubic.m', ...
...
matlabFunction(jac_sig_r_br_cubic,'File','jac_sig_r_br_cubic.m', ...
    'Vars',{r_n, r_e, r_d, v, gamma, xi, w_e, w_n, w_d, ...
    terr_dis, p1_n, p1_e, p1_h, p2_n, p2_e, p2_h, p3_n, p3_e, p3_h, ...
    r_offset, delta_r0, k_r, ...
    sig_r, d_occ, delta_r, vG_sq, vG, vG_n, vG_e, vG_d},'Outputs',{'out'});
...
%% export to c code
if 1
ccode(jac_sig_r_tl_cubic,'file','jac_sig_r_tl_cubic_ccode.c');
ccode(jac_sig_r_br_cubic,'file','jac_sig_r_br_cubic_ccode.c');
end

%% prep c code for mpc model functions
if 1
% br
fid = fopen('jac_sig_r_br_cubic_ccode.c');
...
fid = fopen('jac_sig_r_br_cubic_ccode.c','w');
for k = 1:length(txt)
    fprintf(fid,[char(txt{k}),' \n']);
end
fclose(fid);
% tl
fid = fopen('jac_sig_r_tl_cubic_ccode.c');
txt = textscan(fid,'%s','delimiter','\n'); 
fclose(fid);
txt = txt{1};
for i = 1:length(txt)
...
fid = fopen('jac_sig_r_tl_cubic_ccode.c','w');
for k = 1:length(txt)
    fprintf(fid,[char(txt{k}),' \n']);
end
end
