...
for jj = 1:node_int:(N+1)
    ...
    det(rec.occ_slw(jj+len_sliding_window-1,isp_nmpc:hor_int_mpc:iep_nmpc,OCC_DETECT_FWD)==0) = NaN;
    plot3(...
        [rec.x_hor(jj,hor_indices,2); rec.occ_slw(jj+len_sliding_window-1,isp_nmpc:hor_int_mpc:iep_nmpc,P_OCC_FWD+0)] .* det, ...
        [rec.x_hor(jj,hor_indices,1); rec.occ_slw(jj+len_sliding_window-1,isp_nmpc:hor_int_mpc:iep_nmpc,P_OCC_FWD+1)] .* det, ...
        [-rec.x_hor(jj,hor_indices,3); rec.occ_slw(jj+len_sliding_window-1,isp_nmpc:hor_int_mpc:iep_nmpc,P_OCC_FWD+2)] .* det,'-r');
    quiver3(...
        rec.occ_slw(jj+len_sliding_window-1,isp_nmpc:hor_int_mpc:iep_nmpc,P_OCC_FWD+0) .* det, ...
        rec.occ_slw(jj+len_sliding_window-1,isp_nmpc:hor_int_mpc:iep_nmpc,P_OCC_FWD+1) .* det, ...
        rec.occ_slw(jj+len_sliding_window-1,isp_nmpc:hor_int_mpc:iep_nmpc,P_OCC_FWD+2) .* det, ...
        rec.occ_slw(jj+len_sliding_window-1,isp_nmpc:hor_int_mpc:iep_nmpc,N_OCC_FWD+0) .* det * sc_rays, ...
        rec.occ_slw(jj+len_sliding_window-1,isp_nmpc:hor_int_mpc:iep_nmpc,N_OCC_FWD+1) .* det * sc_rays, ...
        rec.occ_slw(jj+len_sliding_window-1,isp_nmpc:hor_int_mpc:iep_nmpc,N_OCC_FWD+2) .* det * sc_rays,'r','autoscale','off');
    plot3(...
        rec.occ_slw(jj+len_sliding_window-1,isp_nmpc:hor_int_mpc:iep_nmpc,P_OCC_FWD+0) .* det, ...
        rec.occ_slw(jj+len_sliding_window-1,isp_nmpc:hor_int_mpc:iep_nmpc,P_OCC_FWD+1) .* det, ...
        rec.occ_slw(jj+len_sliding_window-1,isp_nmpc:hor_int_mpc:iep_nmpc,P_OCC_FWD+2) .* det,'rs');
end
sc_quiv = 10;
mpc_kk = isp_nmpc:hor_int_mpc:iep_nmpc;
sim_kk = isp:hor_int:iep;
len_kk = length(mpc_kk);
for jj = 1:node_int:N
    for kk = 1:len_kk
        ww = rec.W(jj,mpc_kk(kk),1) / Q_output(1);
        quiver3(...
            rec.x_hor(jj,sim_kk(kk),2), ...
            rec.x_hor(jj,sim_kk(kk),1), ...
            -rec.x_hor(jj,sim_kk(kk),3), ...
            rec.yref(jj,mpc_kk(kk),2)*sc_quiv, ...
            rec.yref(jj,mpc_kk(kk),1)*sc_quiv, ...
            -rec.yref(jj,mpc_kk(kk),3)*sc_quiv, ...
            'autoscale','off','color',ww.*[0 1 0] + (1-ww).*[0.8 0.8 0.8]);
    end
end
for kk = 1:len_kk
    quiver3(...
            rec.x_hor(N+1,sim_kk(kk),2)', ...
            rec.x_hor(N+1,sim_kk(kk),1)', ...
            -rec.x_hor(N+1,sim_kk(kk),3)', ...
            rec.yNref(mpc_kk(kk),2)*sc_quiv, ...
            rec.yNref(mpc_kk(kk),1)*sc_quiv, ...
            -rec.yNref(mpc_kk(kk),3)*sc_quiv, ...
            'autoscale','off','color',ww.*[0 1 0] + (1-ww).*[0.8 0.8 0.8]);
end
