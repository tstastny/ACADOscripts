...
v_x = v * cos(alpha); % velocity in x-direction body frame
v_z = v * sin(alpha); % velocity in z-direction body frame
...
% state differentials
dot_v = (v_x*dot_v_x + v_z*dot_v_z)/(v); % velocity danamics
dot_theta = (k*theta_ref - theta)/tau_0; % theta dynamics
dot_gamma = dot_theta - (v_x*dot_v_z + v_z*dot_v_x)/(v^2); % flight path angle dynamics
dot_zeta_w = c_w*delta_w; % zeta_w dynamics
...
copyfile('../acado/external_packages/qpoases', ...
    'export_nmpc/qpoases')
nmpc.exportCode( 'export_nmpc_path' );

% this actually compiles the code for the mex solver
cd export_nmpc
make_acado_solver('../acado_nmpc_step_path')
cd ..
