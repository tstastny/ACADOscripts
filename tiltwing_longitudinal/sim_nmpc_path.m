...
% initial states
x_init = [15, deg2rad(10), 0.0, deg2rad(15)]; % XXX: put something realistic here!
...
y_ref = [v, gamma, theta_ref, zeta_w_ref, delta_ref, T_w_ref, theta_ref,];

% objective weights
Q_scale = [100, 100, 0.1, 0, 50, 0.1, 0.5];
Q_scale_N = [400, 400, 0.1, 0];

Q = [1, 2500, 2500, 280, 6.5, 25, 2500] .* Q_scale; % XXX: put something realistic here
QN = [1, 2500, 2500, 280] .* Q_scale_N;% << only state dependent (no controls)!

% Q = [10, 10, 30, 20, 15, 1, 30];
% QN = [100, 100, 60, 40];
...
measurements = states;
controls = nmpc_ic.u;
[dstates,simout] = model_dynamics_path(0,measurements,controls); % XXX: define what actual inputs you use, 0 here is t=0
...
for k = 1:len_t
    ...
    if mod(k,nmpc_counter_interval)==0 || k==1 % only execute the mpc at the defined iteration rate
        ...
        if k > 1000
            ...
            gamma = deg2rad(10);
            theta_ref = 0.0;
            zeta_w_ref = deg2rad(15);
            delta_ref = 0.0;
            T_w_ref = 15;

            y_ref = [v_x_ref, gamma,theta_ref, zeta_w_ref, delta_ref, T_w_ref, theta_ref,];
            input.y = repmat(y_ref, N,1);
            input.yN = y_ref(1:n_Ys); % << only state dependent (no controls)!
        end
        ...
        % control - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        % generate controls
        output = acado_nmpc_step_path(input);
    ...
    % apply control
    [dstates, simout]  = model_dynamics_path(time(k), states, controls);
    dstates;
    states;
...
figure
subplot(7,1,1);
plot(time,rad2deg(atan(rec.x(:,2)./rec.x(:,1))));
title('\alpha');
subplot(7,1,2);
plot(time,rad2deg(rec.x(:,4)));
title('\zeta_w');
subplot(7,1,3);
plot(time,rad2deg(rec.x(:,4)) + rad2deg(atan(rec.x(:,2)./rec.x(:,1))));
title('\alpha + \zeta_w');
subplot(7,1,4);
