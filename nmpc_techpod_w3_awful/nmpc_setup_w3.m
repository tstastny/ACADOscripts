...
DifferentialState x_w3_V;      %   (V w3)                [~]
DifferentialState x_w3_theta;      %   (theta w3)               [~]
DifferentialState x_w3_phi;      %   (phi w3)                [~]
DifferentialState x_w3_beta;      %   (beta w3)                 [~]
...
% z_w3_V = Cw2*x_w3_V + Dw3*(V_cmd - V + intg_V);
% z_w3_theta = Cw2*x_w3_theta + Dw3*(theta_cmd - theta);
% z_w3_phi = Cw2*x_w3_phi + Dw3*(phi_cmd - phi);
% z_w3_beta = Cw2*x_w3_beta + Dw3*beta;

z_w3_V = Cw3*x_w3_V + Dw3*(V + intg_V);
z_w3_theta = Cw3*x_w3_theta + Dw3*(theta);
z_w3_phi = Cw3*x_w3_phi + Dw3*(phi);
z_w3_beta = Cw3*x_w3_beta + Dw3*beta;

% OPTIMAL CONTROL PROBLEM -------------------------------------------------

% state output
y = [ V; theta; phi; beta; p; q; r; z_w3_V; z_w3_theta; z_w3_phi; z_w3_beta ];
...
f.add(dot(x_w3_V) == Aw3*x_w3_V + Bw3*(V + intg_V));
f.add(dot(x_w3_theta) == Aw3*x_w3_theta + Bw3*(theta));
f.add(dot(x_w3_phi) == Aw3*x_w3_phi + Bw3*(phi));
f.add(dot(x_w3_beta) == Aw3*x_w3_beta + Bw3*beta);
