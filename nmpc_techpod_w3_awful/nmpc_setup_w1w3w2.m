...
DifferentialState x_w1_V;      %   (throtte w2)                [~]
DifferentialState x_w1_theta;      %   (elevator w2)               [~]
DifferentialState x_w1_phi;      %   (aileron w2)                [~]
DifferentialState x_w1_beta;      %   (rudder w2)                 [~]
...
DifferentialState x_w2_sv;      %   (slack w2)                 [~]

DifferentialState x_w3_p;      %   (V w3)                [~]
DifferentialState x_w3_q;      %   (theta w3)               [~]
DifferentialState x_w3_r;      %   (phi w3)                [~]
...
z_w1_V = Cw1*x_w1_V + Dw1*(V_cmd - V + intg_V);
z_w1_theta = Cw1*x_w1_theta + Dw1*(theta_cmd - theta);
z_w1_phi = Cw1*x_w1_phi + Dw1*(phi_cmd - phi);
z_w1_beta = Cw1*x_w1_beta + Dw1*beta;

z_w3_p = Cw3*x_w3_p + Dw3*p;
z_w3_q = Cw3*x_w3_q + Dw3*q;
z_w3_r = Cw3*x_w3_r + Dw3*r;

% OPTIMAL CONTROL PROBLEM -------------------------------------------------

% state output
y = [ z_w1_V; z_w1_theta; z_w1_phi; z_w1_beta; z_w3_p; z_w3_q; z_w3_r ];
...
f.add(dot(x_w1_V) == Aw1*x_w1_V + Bw1*(V_cmd - V + intg_V));
f.add(dot(x_w1_theta) == Aw1*x_w1_theta + Bw1*(theta_cmd - theta));
f.add(dot(x_w1_phi) == Aw1*x_w1_phi + Bw1*(phi_cmd - phi));
f.add(dot(x_w1_beta) == Aw1*x_w1_beta + Bw1*beta);
...
f.add(dot(x_w3_p) == Aw3*x_w3_p + Bw3*p);
f.add(dot(x_w3_q) == Aw3*x_w3_q + Bw3*q);
f.add(dot(x_w3_r) == Aw3*x_w3_r + Bw3*r);
