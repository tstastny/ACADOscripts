...
const double t10 = in[11]*t3*t9;
const double t2 = in[30]+t10;
const double t7 = sin(in[5]);
const double t8 = in[11]*t3*t7;
const double t4 = in[31]+t8;
const double t5 = sin(in[4]);
const double t11 = in[11]*t5;
const double t6 = in[32]-t11;
...
int pparam_sel = 0;
// if ( in[12] < 0.5 ) {
//     b_switch_segment = check_line_seg( &in[0], &in[12] );
// } else if (in[12] < 1.5 ) {
//     b_switch_segment = check_curve_seg( &in[0], &in[12] );
// }
...
const double pparam_type = in[12+pparam_sel];

// LINE SEGMENT
if ( pparam_type < 0.5 ) {

    // variable definitions
    const double pparam_aa_n = in[13+pparam_sel];
    const double pparam_aa_e = in[14+pparam_sel];
    const double pparam_aa_d = in[15+pparam_sel];
    const double pparam_bb_n = in[16+pparam_sel];
    const double pparam_bb_e = in[17+pparam_sel];
    const double pparam_bb_d = in[18+pparam_sel];
...
// CURVE SEGMENT
    ...
    // variable definitions
    const double pparam_cc_n = in[13+pparam_sel];
    const double pparam_cc_e = in[14+pparam_sel];
    const double pparam_cc_d = in[15+pparam_sel];
    const double pparam_R = in[16+pparam_sel];
    const double pparam_ldir = in[17+pparam_sel];
    const double pparam_gam_sp = in[18+pparam_sel];
    const double pparam_xi0 = in[19+pparam_sel];
    const double pparam_dxi = in[20+pparam_sel];
...
out[3] = in[9];
out[4] = in[10];
out[5] = (tan(in[3])*(9.81E2/1.0E2))/in[11];
out[6] = -1.0/(exp((t12*t12)*(-1.0/1.0E1)-(t14*t14)*(1.0/1.0E1)-(t17*t17)*(1.0/1.0E1)+5.0/2.0)+1.0)+1.0;
out[7] = Gamma_d+asin(sin_d_dot_V_g);
out[8] = e_chi;
...
const double t4 = in[31]+t8;
const double t5 = sin(in[4]);
const double t6 = in[32]-in[11]*t5;
...
out[0] = in[3];
out[1] = in[4];
out[2] = sqrt(t11*t11+t13*t13+t16*t16);
