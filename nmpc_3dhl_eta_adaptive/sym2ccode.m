...
syms T_b_lat;   % (lateral-directional path-following constant) [s]
syms T_b_lon;   % (longitudinal path-following constant) [s]

ctrls   = [u_T,phi_ref,theta_ref,T_b_lat,T_b_lon];
n_U     = length(ctrls);
...
% state output
y   = [ Vsafe; p; q; r; a_soft ];
n_Y = length(y);

% ctrl output
z   = [ eta_lat; eta_lon; T_b_lat; T_b_lon; delta_T_dot; u_T; phi_ref; theta_ref; u_T; phi_ref; theta_ref ];
n_Z = length(z);
