function [out,aux] = calculate_cost(in,Ns)
...
n_dot = in(38+1)+Vsafe*t2*cos(in(5+1));
e_dot = in(39+1)+Vsafe*t2*sin(in(5+1));
d_dot = in(40+1)-Vsafe*sin(in(4+1));
...
if (t19>in(44+1))
    e_b_lat = sqrt(t19)*in(16+1);                               
else
    e_b_lat = in(16+1)*in(44+1)*(1.0/2.0)+in(16+1)*t5*t19*(1.0/2.0);
end
...
if (abs(d_dot)>in(44+1))
    e_b_lon = abs(d_dot)*in(17+1);                               
else
    e_b_lon = in(17+1)*in(44+1)*(1.0/2.0)+in(17+1)*t5*abs(d_dot)*(1.0/2.0);
end
...
t20 = alpha-in(41+1)+in(43+1);
t21 = 1.0/(in(43+1)*in(43+1));
t22 = -alpha+in(42+1)+in(43+1);
...
if (alpha>(in(41+1)-in(43+1)))
    a_soft=(t20*t20)*t21;
elseif (alpha>(in(42+1)+in(43+1)))
    a_soft=0.0;
else
    a_soft=t21*(t22*t22);
end
