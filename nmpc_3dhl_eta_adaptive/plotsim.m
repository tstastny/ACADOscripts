...
hc(1)=subplot(5,1,1:2); hold on; grid on;
[ax,plot1,plot2] = plotyy(time,aux_rec(:,1:2),time,rad2deg(Y_rec(:,6:7)));
legend(ax(1),{'e_{lat}','e_{lon}'})
legend(ax(2),{'\eta_{lat}','\eta_{lon}'})
ylabel(ax(1),'e [m]')
ylabel(ax(1),'\eta [deg]')

hc(3)=subplot(5,1,3:5); hold on; grid on;

plot(time,(yref(1)*ones(length(time),1)-Y_rec(:,1)).^2*Q_output(1)); % e_V
plot(time,(yref(2)*ones(length(time),1)-Y_rec(:,2)).^2*Q_output(2)); % e_p
plot(time,(yref(3)*ones(length(time),1)-Y_rec(:,3)).^2*Q_output(3)); % e_q
plot(time,(yref(4)*ones(length(time),1)-Y_rec(:,4)).^2*Q_output(4)); % e_r
plot(time,(yref(5)*ones(length(time),1)-Y_rec(:,5)).^2*Q_output(5)); % alpha_soft
plot(time,(zref(6)*ones(length(time),1)-Y_rec(:,6)).^2*R_controls(1)); % eta_lat
plot(time,(zref(7)*ones(length(time),1)-Y_rec(:,7)).^2*R_controls(2)); % eta_lon

legend('e_V','e_p','e_q','e_r','\alpha_{soft}','\eta_{lat}','\eta_{lon}')
ylabel('J(x,u)')
...
hl = gobjects(0);
hl(length(hl)+1) = plot(time,yref(1)*ones(1,length(time)),'color',c_ref);
...
plot(time,rad2deg(aux_rec(:,1)));
plot(time,rad2deg(aux_rec(:,2)));
ylabel('\eta [deg]');
