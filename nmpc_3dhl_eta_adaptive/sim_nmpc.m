...
T_b_lon = 3.5;                    % longitudinal track-error boundary constant
...
ic_att  = [0, 0];
ic_attdot = [0, 0, 0];
ic_u    = [0.375, 0, 1.7*pi/180, T_b_lat T_b_lon];
ic_augm = [ic_u(1), 0];
...
nmpc_ic.u   = ic_u;
yref        = [14, 0 0 0, 0];
zref        = [0 0, ic_u(4:5), 0, ic_u(1:3), ic_u(1:3)];

Q_scale     = [1, 50*pi/180 50*pi/180 50*pi/180, 1];
R_scale     = [pi/2 pi/2, 1 1, 1, 1 30*pi/180 15*pi/180, 1 5*pi/180 5*pi/180];

Q_output    = [10, 20 20 5, 1]./Q_scale.^2;
QN_output   = [10, 20 20 5, 1]./Q_scale.^2;
R_controls  = [150 100, 0.5 0.5, 40 0.1 0.1 0.1 30 20 30]./R_scale.^2;
...
for i = 1:N
input.W(n_Y*(i-1)+1:n_Y*i,:) = diag([Q_output, R_controls(1:8), R_controls(9:end).*Q_prev(i,:)]);
end
...
for k = 1:length(time)
    ...
    if time(k)==floor(time(k)/Ts_nmpc)*Ts_nmpc
        ...
        if k > 1
            ...
            input.y(:,(end-2):end) = output.u(:,1:3);%[output.u(1,1)*ones(N,1),output.u(:,2:3)];%repmat(output.u(1,:),N,1);%
        end
    ...
    % apply control
    [d_states, simout]  = uav_dyn(time(k), states, U0(1:3), [wn,we,wd], parameters);
    ...
    [out,aux] = calculate_cost([X0,U0,ic_od],[n_X,n_U]);
    aux_rec(k,:) = aux;
    Y_rec(k,:) = out;
