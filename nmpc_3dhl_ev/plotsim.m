...
if true
    ...
    quiver3(aux_rec(1:guide_disp_int:end,2),aux_rec(1:guide_disp_int:end,1),-aux_rec(1:guide_disp_int:end,3),...
        aux_rec(1:guide_disp_int:end,5),aux_rec(1:guide_disp_int:end,4),-aux_rec(1:guide_disp_int:end,6),'c')
end
% position horizons
if horiz_disp
...
plot(time,Y_rec(:,3)); % v_lat
plot(time,Y_rec(:,4)); % v_lon
...
plot(time,(yref(3)*ones(length(time),1)-Y_rec(:,3)).^2*Q_output(3)); % e_v_lat
plot(time,(yref(4)*ones(length(time),1)-Y_rec(:,4)).^2*Q_output(4)); % e_v_lon
plot(time,(yref(5)*ones(length(time),1)-Y_rec(:,5)).^2*Q_output(5)); % V
plot(time,(yref(6)*ones(length(time),1)-Y_rec(:,6)).^2*Q_output(6)); % p
plot(time,(yref(7)*ones(length(time),1)-Y_rec(:,7)).^2*Q_output(7)); % q
plot(time,(yref(8)*ones(length(time),1)-Y_rec(:,8)).^2*Q_output(8)); % r
plot(time,(yref(9)*ones(length(time),1)-Y_rec(:,9)).^2*Q_output(9)); % alpha_soft
...
legend('e_{lat}','e_{lon}','e_{v_{lat}}','e_{v_{lon}}','V','p','q','r','\alpha_{soft}','u_T','\phi_{ref}','\theta_{ref}')
ylabel('J(x,u)')
...
plot(time,yref(5)*ones(1,length(time)),'color',c_ref);
plot(time,X_rec(:,4));
plot(time,sqrt(aux_rec(:,8).^2 + aux_rec(:,9).^2));
plot(time,aux_rec(:,10));
plot(time,sqrt(aux_rec(:,11).^2 + aux_rec(:,12).^2 + aux_rec(:,13).^2));
