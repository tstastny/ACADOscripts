...
// CURVE SEGMENT
    ...
    if (fabs(pparam_gam_sp) < 0.001) {
        ...
        // nearest spiral leg
        delta_d_k = round( (in[2] - (pparam_cc_d + delta_d_xi)) / (6.28318530718*Rtangam) ) * 6.28318530718*Rtangam;

        // closest point on nearest spiral leg
        d_d = pparam_cc_d + delta_d_k + delta_d_xi;

        /* d (on spiral) = (start height) + (revolution height
         * increment) + (lateral-direcitonal angular position increment) */

        // cap end point
        if ( (d_d - (delta_d_sp_end + pparam_cc_d)) * pparam_gam_sp < 0.0 ) {
            // we are beyond the end point
            
            d_d = delta_d_sp_end + pparam_cc_d;
            Td_d = 0.0;
        }
        else {
