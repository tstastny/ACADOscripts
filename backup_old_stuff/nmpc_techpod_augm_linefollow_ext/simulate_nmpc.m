...
for k = 1:length(time)
    ...
    % guidance
    aa_n = paths(pth_idx).aa(1);
    aa_e = paths(pth_idx).aa(2);
    aa_d = paths(pth_idx).aa(3);
    
    bb_n = paths(pth_idx).bb(1);
    bb_e = paths(pth_idx).bb(2);
    bb_d = paths(pth_idx).bb(3);

    input.od    = repmat([L1p_lon, L1d_lon, L1p_lat, L1d_lat, kilon, kilat, ...
    kiV, V_cmd, aa_n, aa_e, aa_d, bb_n, bb_e, bb_d, ...
    Aw2, Bw2, Cw2, Dw2, wn, we, wd],N+1,1);
