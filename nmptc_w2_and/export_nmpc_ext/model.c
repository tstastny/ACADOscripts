...
const double t10 = in[15]*t3*t9;
const double t2 = in[34]+t10;
const double t7 = sin(in[5]);
const double t8 = in[15]*t3*t7;
const double t4 = in[35]+t8;
const double t5 = sin(in[4]);
const double t11 = in[15]*t5;
const double t6 = in[36]-t11;
...
out[3] = in[6];
out[4] = in[7];
out[5] = (tan(in[3])*(9.81E2/1.0E2))/in[15];
out[6] = -in[37]*(in[6]*in[38]*2.0+in[37]*(in[3]-in[13]));
out[7] = -in[39]*(in[7]*in[40]*2.0+in[39]*(in[4]-in[14]));
out[8] = in[8]*in[41]+in[13]*in[42];
out[9] = in[9]*in[41]+in[14]*in[42];
out[10] = sqrt(t12*t12+t14*t14+t17*t17);
out[11] = Gamma_d+asin(sin_d_dot_V_g);
out[12] = e_chi;
...
const double t4 = in[35]+t8;
const double t5 = sin(in[4]);
const double t6 = in[36]-in[15]*t5;
...
out[2] = in[6];
out[3] = in[7];
out[4] = sqrt(t11*t11+t13*t13+t16*t16);
...
out[10] = in[13];
out[11] = in[14];
out[12] = in[8]*in[43]+in[13]*in[44];
out[13] = in[9]*in[43]+in[14]*in[44];
