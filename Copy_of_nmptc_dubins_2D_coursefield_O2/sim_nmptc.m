...
states      = simout;
d_states    = [...
    ic_V*cos(ic_att(1))+wn, ...
    ic_V*sin(ic_att(1))+we, ...
    zeros(1,length(ic_att)+length(ic_attdot))];
cost = 0;
U0 = 0;
path_idx = 1;
...
%     X0(10:15)   = output.x(2,10:15);
    ...
    % apply control
    [d_states, simout, cost]  = uav3DoF(time(k), states, U0, wn, we, ic_V, pparams);
