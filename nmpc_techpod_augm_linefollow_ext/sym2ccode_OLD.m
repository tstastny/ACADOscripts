...
assumeAlso(L1_lon > abs(xtrackerr_lon))
assumeAlso(L1_lon > 0);
...
etalon = atan2(d_dot, V_lat) - atan2(LL_lon_d, LL_lon_ne);
...
etalonPI    = etalon1 * KL1_lon + intg_lon;

% guidance commands lateral_accel = K_L1 * ground_speed / L1_ratio * sin(eta);
theta_cmd   = atan2(V_lon * etalonPI, L1R_lon * g);
...
pp_proj     = aa_bb_lat_unit_n*aa_pp_n + aa_bb_lat_unit_e*aa_pp_e;

dd_lat_n        = aa_n + pp_proj * aa_bb_lat_unit_n;
dd_lat_e        = aa_e + pp_proj * aa_bb_lat_unit_e;
...
% lateral error angle
etalat = atan2(LL_lat_e, LL_lat_n) - atan2(e_dot, n_dot);
...
% jacobians
% Jrhs	= reshape(transpose(jacobian(frhs(14),[states,ctrls])),1*(n_X+n_U),1);	% rhs
...
% % % % % Jrhs = subs( Jrhs, [states ctrls onlinedata], ins );
% % % % % objectives = subs( objectives, [states ctrls onlinedata], ins );
% % % % % Jobj_x = subs( Jobj_x, [states ctrls onlinedata], ins );
% % % % % Jobj_u = subs( Jobj_u, [states ctrls onlinedata], ins );
