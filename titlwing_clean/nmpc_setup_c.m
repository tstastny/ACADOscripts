...
tau_T = 0.05;   % control augmented time constant of Thrust
...
% state differentials
dot_v_x = 1/m*(T*cos(zeta_w)-m*g*sin(theta)-k_T2L*T*sin(zeta_w)...
    +0.5*rho*(v_x^2+v_z^2)*S_w*(C_Ltotal(alpha+zeta_w)*sin(alpha)...
    -C_Dtotal(alpha+zeta_w)*cos(alpha)))-(k*theta_ref - theta)/tau_0*v_z;
dot_v_z = 1/m*(-T*sin(zeta_w)+m*g*cos(theta)-k_T2L*T*cos(zeta_w)...
...
% y_s = [v_x; v_z; theta; zeta_w; T];

% control dependent objectives
% y_c = [delta_w; T_ref; theta_ref];
...
cd export_nmpc
make_acado_solver('../acado_nmpc_step', 'lsq_objective_tiltwing.c')
