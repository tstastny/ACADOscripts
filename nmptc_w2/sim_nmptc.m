...
ic_u    = [0,0];
ic_augm = [0,0,0];
ic_od   = [ic_V, pparams, pparams_next, wn, we, wd, aw2, bw2, cw2, dw2];
...
% initial simout
X0          = nmpc_ic.x;
simout      = [nmpc_ic.x(1:3) 0 0 nmpc_ic.x(4)];
states      = simout;
d_states    = [...
...
%     X0(10:15)   = output.x(2,10:15);
    ...
    Horiz_mu_rec(k,:) = output.u(:,1)';
    Horiz_gam_rec(k,:) = output.u(:,2)';
...
%     Y_rec(k,:)	= [Va_cmd, theta_cmd, phi_cmd, intg_lon, intg_lat, etalon, etalat];
    ...
    X0(1:(n_X-n_XA)) = [simout(1:3) simout(6)];
