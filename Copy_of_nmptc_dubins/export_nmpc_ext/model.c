...
out[6] = -in[34]*(in[6]+in[32]*(in[3]-in[8]));
out[7] = -in[35]*(in[7]+in[33]*(in[4]-in[9]));
...
const double t4 = in[30]+t8;
const double t5 = sin(in[4]);
const double t6 = in[31]-in[10]*t5;
...
const double pparam_type = in[11+pparam_sel];

// LINE SEGMENT
if ( pparam_type < 0.5 ) {

    // variable definitions
    const double pparam_aa_n = in[12+pparam_sel];
    const double pparam_aa_e = in[13+pparam_sel];
    const double pparam_aa_d = in[14+pparam_sel];
    const double pparam_bb_n = in[15+pparam_sel];
    const double pparam_bb_e = in[16+pparam_sel];
    const double pparam_bb_d = in[17+pparam_sel];
...
// CURVE SEGMENT
    ...
    // variable definitions
    const double pparam_cc_n = in[12+pparam_sel];
    const double pparam_cc_e = in[13+pparam_sel];
    const double pparam_cc_d = in[14+pparam_sel];
    const double pparam_R = in[15+pparam_sel];
    const double pparam_ldir = in[16+pparam_sel];
    const double pparam_gam_sp = in[17+pparam_sel];
    const double pparam_xi0 = in[18+pparam_sel];
    const double pparam_dxi = in[19+pparam_sel];
